...
- Export supports HTML, PNG, image-based PDF, and Word `.docx`, including relative local images, `data:image`, HTTP(S), SVG/GIF/WebP conversion, and save-location reveal; PDF and Word embed image assets so shared files remain viewable offline.
- Copy to WeChat Official Account, Zhihu, and Juejin writes rich HTML clipboard formats, including `text/html`, macOS `public.html`, and native Windows `HTML Format`, with the active typography theme and compact layout converted to inline styles.
...
| Export | HTML, PNG, image-based PDF, Word `.docx`, and print preview; PDF/Word embed local, `data:image`, HTTP(S), SVG/GIF/WebP images. |
| Publishing copy | Rich HTML clipboard payloads for WeChat Official Account, Zhihu, and Juejin. Windows `HTML Format` uses UTF-8 CF_HTML bytes and applies the active typography theme. |
