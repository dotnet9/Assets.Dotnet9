...
当前截图对应的仓库版本已继续更新到 `12.0.3.12`。我本地已经执行过：
...
测试项目当前 41 个用例通过。构建时会看到 .NET 预览版 SDK 的提示，但没有编译警告和错误。
...
- 改进图片资源释放和图片预览窗口资源持有。
- 新增共享图片加载与栅格化能力，以及 `MarkdownDocumentExporter`/`ExportKind`，方便宿主应用用一个公共入口导出 PNG、图像型 PDF、Word，并嵌入图片。
