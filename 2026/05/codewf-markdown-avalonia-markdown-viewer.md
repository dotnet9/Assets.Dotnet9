...
当前截图对应的仓库版本是 `12.0.3.9`。我本地已经执行过：
...
测试项目当前 24 个用例通过。构建时会看到 .NET 预览版 SDK 的提示，但没有编译警告和错误。
...
除了预览控件本身，`CodeWF.Markdown` 现在还提供了 `MarkdownHtmlClipboard`。它不是把 Markdown 原文塞进剪贴板，而是给宿主应用写入可粘贴到网页编辑器的富 HTML：同时提供 `text/html`、macOS `public.html` 和 Windows 原生 `HTML Format`。

这里最关键的是 Windows `HTML Format`。微信公众号、知乎、稀土掘金这类编辑器通常跑在 Chromium 系浏览器里，如果剪贴板格式不对，就会把 HTML 片段当普通文本显示出来。`MarkdownHtmlClipboard` 会按 UTF-8 字节计算 CF_HTML 的 `StartHTML`、`EndHTML`、`StartFragment`、`EndFragment` 偏移，并处理 `<!--StartFragment-->` / `<!--EndFragment-->` 片段标记。Vex 的“复制到公众号 / 知乎 / 稀土掘金”已经切到这套公共能力上。
...
- 改进图片资源释放和图片预览窗口资源持有。
- 新增共享图片加载与栅格化能力，方便宿主应用在 PDF、PNG、Word 等导出链路中嵌入图片。
- 新增 `MarkdownHtmlClipboard`，为微信公众号、知乎、稀土掘金等网页编辑器提供富 HTML 剪贴板载荷。
