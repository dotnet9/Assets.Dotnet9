...
当前截图对应的仓库版本已继续更新到 `12.0.3.14`。我本地已经执行过：
...
测试项目当前 48 个用例通过。构建时会看到 .NET 预览版 SDK 的提示，但没有编译警告和错误。
...
另外，网页内容粘贴回 Markdown 编辑器的方向也放进了公共库。宿主应用可以先读取剪贴板里的 HTML，再调用：

```csharp
var markdown = MarkdownHtmlClipboard.Html2Markdown(htmlContent);
```

这个 API 会把网页复制得到的标题、段落、链接、图片、列表、引用、代码块和表格转成 Markdown。Vex 中间编辑器现在就是先尝试这条路径；没有 HTML 或转换失败时，再回落到普通粘贴。
...
- 新增 `MarkdownHtmlClipboard.Html2Markdown(htmlContent)`，让宿主编辑器可以把从网页复制得到的 HTML 自动转为 Markdown 后再粘贴。
