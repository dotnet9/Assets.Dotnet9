...
- 本地相对图、`data:image`、HTTP(S)、SVG/GIF/WebP 转 PNG、任务列表、排版主题化导出、PDF 页眉页脚；PDF 和 Word 会嵌入图片资源，离线分享后仍可查看。
...
- 复制到公众号、知乎、稀土掘金把“发布到哪里”也放进编辑流程；剪贴板会写入 `text/html`、macOS `public.html` 和 Windows 原生 `HTML Format`，并把当前排版主题和紧凑布局转换为 inline style。
- 导出菜单支持 HTML、PNG、图像型 PDF 和 Word `.docx`；本地相对图、`data:image`、HTTP(S)、SVG/GIF/WebP 转 PNG 等边界会随导出一起处理，PDF 和 Word 会嵌入图片资源。

对公众号、知乎、掘金作者来说，真正高频的不是“保存一个 HTML 文件”，而是写完后直接粘贴到网页编辑器，并尽量保留标题、段落、引用、列表、代码块、表格和链接样式。Vex 当前复用 `CodeWF.Markdown.MarkdownHtmlClipboard` 生成 UTF-8 CF_HTML 字节载荷，避免编辑器把 HTML 片段当明文显示。
...
- 继续优化复制到微信公众号、知乎、稀土掘金的 HTML 结构和样式兼容性。
- 增加公众号移动端预览效果。
