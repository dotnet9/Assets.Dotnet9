...
本轮截图和 GIF 已按当前界面重新制作。程序启动时默认进入空白脑图；需要展示文件列表、小图、缩放、画布拖拽和层级调整时，可以手动打开随程序输出的使用手册。

![](https://img1.dotnet9.com/2026/05/zhijian-main-window.gif)

## 启动默认空白，手册可主动打开

新版本启动后会默认创建只有中心主题的空白脑图，避免用户一打开软件就被示例内容打断。随程序输出的 `使用手册.md` 仍然保留，它本身是一棵多层级脑图，需要学习或验证复杂结构时，可以通过“文件 -> 打开”主动载入，观察文件列表、大纲和脑图的联动效果。
...
这轮不只是改完代码看一眼编译。我重新生成了文档截图/GIF，并用手动打开的使用手册覆盖这些操作：

- 启动默认进入空白脑图，并确认手动打开的文件会出现在左侧文件列表。
